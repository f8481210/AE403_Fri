...
class Game:
    ...
    def key_handler(self, event):
        """先判斷遊戲是否終止"""
        if self.is_game_terminated(): #368
            return
        """每次點擊案件先清除網格的旗標(flags)"""
        self.grid.clear_flags() #不可壓縮也不可合併 55
        """設定變數儲存事件抓到的按鍵"""
    ...
    def you_win(self):
        self.won = True
        messagebox.showinfo(title="win",message=str(grid.score)+" 恭喜，你贏了！")
        #print('You Win!')

    """遊戲失敗"""
    def game_over(self):
        messagebox.showinfo(title="gameover",message=str(grid.score)+" 輸了QQ")
        #print('Game over!')
